...
function SortableTable(param)
{
    ...
    this.reRender = function() {
      ...
      for(var columnOrderIdx=0;columnOrderIdx<columnOrder.length;columnOrderIdx++){
    	...
    	mhvstr += "<tbody style='top:38px;' id='"+this.tableid+DELIMITER+"mhvbody'>";
    	for (var i = 0; i < tbl.tblbody.length; i++) {
              ...
              for(var columnOrderIdx=0;columnOrderIdx<columnOrder.length;columnOrderIdx++){
                    ...
                    var cellid = "r"+i+DELIMITER+this.tableid+DELIMITER+columnOrder[columnOrderIdx];
                            ...
                            if {
          					mhvstr += "<td style='white-space:nowrap;' id='"+cellid+DELIMETER+"mhv' onclick='clickedInternal(event,this);' class='"+this.tableid+"-"+columnOrder[columnOrderIdx]+"'>"+renderCell(columnOrder[columnOrderIdx],tbl.tblbody[i][columnOrder[columnOrderIdx]],cellid)+"</td>";
                            }
        				}
      			}
      			str += "</tr>";
