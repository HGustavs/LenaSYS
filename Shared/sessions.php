...
function getQuestion($username)
{
      ...
      $query = $pdo->prepare("SELECT uid,username,superuser,securityquestion,requestedpasswordchange FROM lenasys_user WHERE username=:username LIMIT 1");
...
function checkAnswer($username, $securityquestionanswer)
{
        ...
        //Gets info for the user whose username has been input
        $query = $pdo->prepare("SELECT uid,username,securityquestionanswer FROM lenasys_user WHERE username=:username LIMIT 1");
        ...
        if($query->rowCount() > 0) {
            ...
            if (password_verify($securityquestionanswer, $row['securityquestionanswer'])){
                  ...
                  // The php password is not up to date, update it to be even safer (the cost may have changed, or another algoritm than bcrypt is used)
                    ...
                    $query = $pdo->prepare("UPDATE lenasys_user SET securityquestionanswer = :sqa WHERE uid=:uid");
                    $query->bindParam(':uid', $row['uid']);
...
function requestChange($username)
{
    ...
    $query = $pdo->prepare("UPDATE lenasys_user set requestedpasswordchange=1 where username=:username;");
    $query->bindParam(':username', $username);
...
function login($username, $password, $savelogin)
{
    ...
    if(MYSQL_VERSION<"8.0"){
        $query = $pdo->prepare("SELECT uid,username,password,superuser,lastname,firstname,securityquestion,password(:pwd) as mysql_pwd_input FROM lenasys_user WHERE username=:username LIMIT 1");
        $query->bindParam(':pwd', $password);
    }else{
        $query = $pdo->prepare("SELECT uid,username,password,superuser,lastname,firstname,securityquestion,NULL as mysql_pwd_input FROM lenasys_user WHERE username=:username LIMIT 1");
    }
    $query->bindParam(':username', $username);
    ...
    if($query->rowCount() > 0) {
        ...
        if ($row['password'] == $row['mysql_pwd_input']) {
            ...
            if($newpassword!="UNK"){
                ...
                $query = $pdo->prepare("UPDATE lenasys_user SET password = :pwd WHERE uid=:uid");
                $query->bindParam(':uid', $row['uid']);
...
function isSuperUser($userId)
{
        ...
        $query = $pdo->prepare('SELECT count(uid) AS count FROM lenasys_user WHERE uid=:1 AND superuser=1');
