...
if(checklogin()) {
	if (isSuperUser($userid)) {
		...
		$querystring="SELECT password FROM lenasys_user WHERE uid=:userid LIMIT 1";	
				...
				if(!$query->execute()) {
					...
					if($query->rowCount() > 0) {
						...
						if($action == "challenge"){
								...
								$querystringz = "UPDATE lenasys_user SET securityquestion=:SQ, securityquestionanswer=:answer WHERE uid=:userid";
							...
							//Update password
							$passwordquery = "UPDATE lenasys_user SET password=:PW WHERE uid=:userid";
