...
function updateContent(file, content, boxnumber, titlename) 
{
	...
	if(file != null && box != null && titlename != null){
		filename = file;
		boxtitle = titlename;
		boxcontent = content;
	}
	...
	if ($("#" + contentid + "menu").length == 0) {
		...
		if (retData['writeaccess'] == "w") {
			if (type == "DOCUMENT") {
				...
				str += '<td id = "boxtitlewrapper" class="butto2 boxtitlewrap" title="Title"><span id="boxtitle2" class="boxtitleEditable" onblur="updateContent(' + filename + ','+ filekind +',' + boxid + ',$(this).text());" contenteditable>' + retData['box'][boxid - 1][4] + '</span></td>';
