...
{   
   ...
   if (line.ctype == "BT"){
    ...
    if (line.label && line.label != "" && line.type !== "IE"){
        ...
        if (contextLine.includes(line)) {
            if (line.kind === "Recursive") {
                str += `<text class="cardinalityLabelText" dominant-baseline="middle" text-anchor="middle" style="font-size:${Math.round(zoomfact * textheight)}px;" x="${(labelPositionX*1.3) + textWidth/2 + 2 * zoomfact}" y="${labelPositionY + ((textheight/2 + 2) * zoomfact) + lineLabel.labelMovedY + lineLabel.displacementY}">${line.label}</text>`;
            } else {
