...
var defaults = {
    ...
    SDState: { name: "State", kind: "SDState", fill: "#ffffff", stroke: "#000000", width: 200, height: 50, type: "SD", attributes: ['do:'], functions: [''] }, //<-- SD functionality
...
function drawElement(element, ghosted = false)
{
    ...
    // Check if element is SDState
        ...
        if (elemAttri != 0) {
            ...
            stroke-width='${linew}' stroke='${element.stroke}' fill='${element.fill}' />`;
            for (var i = 0; i < elemAttri; i++) {
                str += `<text x='${xAnchor}' y='${hboxh + boxh * i / 2}' dominant-baseline='middle' text-anchor='${vAlignment}'>${element.attributes[i] + element.functions[i]}</text>`;
            }
            //end of svg for background
            str += `</svg>`;
