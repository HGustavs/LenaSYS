    ...
    if (line.label && line.label != "" && line.type !== "IE"){
        ...
        if (contextLine.includes(line)) {
            if (line.kind === "Recursive") {
                ...
                str += `<rect class="text cardinalityLabel" id=${line.id + "Label"} x="${((fx + length + (40 * zoomfact)))-textWidth/2}" y="${labelPositionY-((textheight/4)*zoomfact)}" width="${(textWidth + zoomfact * 4)}" height="${textheight * zoomfact + zoomfact * 3}"/>`;
