...
function drawElement(element, ghosted = false)
{
    ...
    //=============================================== <-- Start Note functionality
        ...
        str += `<div id='${element.id}'	class='element' onmousedown='ddown(event);' onmouseenter='mouseEnter();' onmouseleave='mouseLeave()';'
        style='left:0px; top:0px;margin-top:${((boxh * -0.25))}px; height:${NOTEEntityHeight}px; width:${boxw}px;font-size:${texth}px;background: linear-gradient(-135deg, transparent ${boxh / 2}px, ${noteFill} 0);`;
        if (context.includes(element)) {
            str += `z-index: 1;`;
        }
        if (ghosted) {
