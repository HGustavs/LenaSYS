...
function drawElement(element, ghosted = false)
{
    ...
    // Sequence loop 
        ...
        str += `<path 
            d="M${(linew+sequenceCornerRadius)+(boxw/15)},${linew}
                v${boxh/10}
                l${((boxw/40))*-1},${(boxh/20)}
                H${linew}
            "
            ...
            fill='none'
        />`;
        str += `<text class='text' x='${((linew+sequenceCornerRadius)+(boxw/15))/2}' y='${(boxh*0.075)+linew}' dominant-baseline='middle' text-anchor='${vAlignment}'>${element.name}</text>`;
        str += `</svg>`;
    }
