...
function mup(event)
{
    ...
    // If no elements are selected - only move one item
    //else 
    //{
    //    // Only update the current hovered element (redundant?)

    //    setPos(elementid, deltaX, deltaY);
    //    console.log("REDUNDANT TEST");
    //}
...
//-------------------------------------------------------------------------------------------------
function setPos(id, deltaX, deltaY)
{
    var index = findIndex(data, id);
    if (index != -1)
    {
        data[index].x -= (deltaX / zoomfact);
        data[index].y -= (deltaY / zoomfact);
    }
}
...
function updateSelection(ctxelement, x, y)
{
    ...
    if (ctxelement != null)
    {
        ...
        if (!context.includes(ctxelement))
        {
            ...
            console.log("Selected: " + ctxelement);
        }
    }
    ...
    else
    {
        ...
        context = [];
        console.log("Clearsed Selection");
    }
}
    ...
    for (var i = 0; i < data.length; i++)
    {
        ...
        if (elementDiv != null)
        {
            ...
            if (deltaX != null && findIndex(context, element.id) != -1)
            {
                ...
                // TODO : This should be re-made into specifics regarding each element type. Current version is simply the "basic" version.
