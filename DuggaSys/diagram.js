    ...
    if (line.label && line.label != "" && line.type !== "IE"){
        ...
        if (contextLine.includes(line)) {
            if (line.kind === "Recursive") {
                ...
                str += `<rect class="text cardinalityLabel" id=${line.id + "Label"} x="${((fx + length + (40 * zoomfact)))-textWidth/2}" y="${(labelPositionY-105)-((textheight/4)*zoomfact)}" width="${(textWidth + zoomfact * 4)}" height="${textheight * zoomfact}"/>`;
                str += `<text class="cardinalityLabelText" dominant-baseline="middle" text-anchor="middle" x="${(fx + length + (40 * zoomfact))}" y="${(labelPositionY-105)+((textheight/4)*zoomfact)}" style="fill:${lineColor}; font-size:${Math.round(zoomfact * textheight)}px;">${line.label}</text>`;
            } else {
