...
var defaults = {
    ...
    ERRelation: { name: "Relation", kind: "ERRelation", fill: "#ffffff", stroke: "#000000", width: 60, height: 60, type: "ER", state: 'normal', canChangeTo: Object.values(relationType) },
...
function drawLine(line, targetGhost = false)
{
   ...
   if (line.ctype == "BT"){
        fy = felem.y2;
        if (felem.kind == elementTypesNames.EREntity) fx = felem.x1 + (((felem.x2 - felem.x1) / (felem.bottom.length + 1)) * (felem.bottom.indexOf(line.id) + 1));
        ty = telem.y1;
    }else if (line.ctype == "TB"){
        fy = felem.y1;
        if (felem.kind == elementTypesNames.EREntity) fx = felem.x1 + (((felem.x2 - felem.x1) / (felem.top.length + 1)) * (felem.top.indexOf(line.id) + 1));
        ty = telem.y2;
    }else if (line.ctype == "RL"){
        fx = felem.x2;
        if (felem.kind == elementTypesNames.EREntity) fy = felem.y1 + (((felem.y2 - felem.y1) / (felem.right.length + 1)) * (felem.right.indexOf(line.id) + 1));
        tx = telem.x1;
    }else if (line.ctype == "LR"){
        fx = felem.x1;
        if (felem.kind == elementTypesNames.EREntity) fy = felem.y1 + (((felem.y2 - felem.y1) / (felem.left.length + 1)) * (felem.left.indexOf(line.id) + 1));
        tx = telem.x2;
    }
            ...
            for (var j = 0; j < lines.length; j++) {
                ...
                if (fElement0.id == fElement.id && fElement0.kind == elementTypesNames.ERAttr && tElement0.kind == elementTypesNames.ERAttr && tElement0.id != tElement.id) {
                    for (var k = 0; k < lines.length; k++) {
                        ...
                        if (fElement1.id == tElement0.id && fElement1.kind == elementTypesNames.ERAttr && (tElement1.kind == elementTypesNames.EREntity || tElement1.kind == "ERRelation")) {
                            errorData.push(element);
                        }
                        if (tElement1.id == tElement0.id && tElement1.kind == elementTypesNames.ERAttr && (fElement1.kind == elementTypesNames.EREntity || fElement1.kind == "ERRelation")) {
                            errorData.push(element);
                        }
                    }
                }
                if (tElement0.id == fElement.id && tElement0.kind == elementTypesNames.ERAttr && fElement0.kind == elementTypesNames.ERAttr && fElement0.id != tElement.id) {
                    for (var k = 0; k < lines.length; k++) {
                        ...
                        if (fElement1.id == fElement0.id && fElement1.kind == elementTypesNames.ERAttr && (tElement1.kind == elementTypesNames.EREntity || tElement1.kind == "ERRelation")) {
                            errorData.push(element);
                        }
                        if (tElement1.id == fElement0.id && tElement1.kind == elementTypesNames.ERAttr && (fElement1.kind == elementTypesNames.EREntity || fElement1.kind == "ERRelation")) {
                            errorData.push(element);
                        }
                    }
                }
