...
function drawElement(element, ghosted = false)
{
    ...
    //=============================================== <-- Start Note functionality
        ...
        if (elemAttri != 0) {
            ...
            str += `<path d="m${linew},${linew} h0 v${boxh / 2 + (boxh * elemAttri / 2) - (linew * 2)} h${boxw - (linew * 2)} v${(boxh / 2 + (boxh * elemAttri / 2) - (linew * 2)) * (-2 / 3)} l${(boxh / 2 + (boxh * elemAttri / 2) - (linew * 2)) * (-1 / 5)}, ${(boxw - (linew * 2))*(-1/3)} stroke-width='${linew}' stroke='${element.stroke}' fill='${element.fill}/>`
            for (var i = 0; i < elemAttri; i++) {
              str += `<text class='text' x='0.5em' y='${hboxh + boxh * i / 2}' dominant-baseline='middle' text-anchor='right'>${text[i]}</text>`;
            }            
