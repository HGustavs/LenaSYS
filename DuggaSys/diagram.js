     ...
     switch (mouseMode) {
        ...
        case mouseModes.PLACING_ELEMENT:
            if (ghostElement) {
                ...
                if (diagramSettings.snapToGrid && mouseMode != mouseModes.EDGE_CREATION){
                    ghostElement.x = Math.round(cords.x / diagramSettings.gridSize) * diagramSettings.gridSize - (ghostElement.width / 2);
                    ghostElement.y = Math.round(cords.y / diagramSettings.gridSize) * diagramSettings.gridSize - (ghostElement.height / 2);
                }else {
...
function makeRandomID()
{
    ...
    while(true) {
        ...
        if (diagramSettings.randomidArray === undefined || diagramSettings.randomidArray.length == 0) { //always add first id
            diagramSettings.randomidArray.push(str);
            return str;

        } else {
            var check = diagramSettings.randomidArray.includes(str); //if check is true the id already exists
            if(check == true){
                str = "";
            } else {
                diagramSettings.randomidArray.push(str);
...
function screenToDiagramCoordinates(mouseX, mouseY)
{
    ...
    // ZOOM IN
    if (zoomfact == 1.25) zoomX = zoomOffsettings.zoom1_25;
    if (zoomfact == 1.5) zoomX = zoomOffsettings.zoom1_5;
    if (zoomfact == 2) zoomX = zoomOffsettings.zoom2;
    if (zoomfact == 4) zoomX = zoomOffsettings.zoom4;

    // ZOOM OUT
    if (zoomfact == 0.75) zoomX = zoomOffsettings.zoom0_75;
    if (zoomfact == 0.5) zoomX = zoomOffsettings.zoom0_5;
    if (zoomfact == 0.25) zoomX = zoomOffsettings.zoom0_25;
    if (zoomfact == 0.125) zoomX = zoomOffsettings.zoom0_125;
...
function setPos(id, x, y)
{
    ...
    if (foundId != -1) {
        ...
        if (diagramSettings.snapToGrid) {
            if (!ctrlPressed) {
                // Calculate nearest snap point
                obj.x = Math.round((obj.x - (x * (1.0 / zoomfact))) / diagramSettings.gridSize) * diagramSettings.gridSize;
                obj.y = Math.round((obj.y - (y * (1.0 / zoomfact))) / diagramSettings.gridSize) * diagramSettings.gridSize;
...
function toggleSnapToGrid()
{
    ...
    // Toggle the boolean
    diagramSettings.snapToGrid = !diagramSettings.snapToGrid;
}
...
function zoomin(scrollEvent = undefined)
{
    ...
    if (scrollEvent && zoomfact <= 4.0) {
        ...
        if(zoomfact < 4.0) { // Only change zoomOrigo when not fully zoomed in.
            zoomOrigo.x += delta.x * zoomOffsettings.zoomPower;
            zoomOrigo.y += delta.y * zoomOffsettings.zoomPower;
        }
...
function updateGridSize()
{
    ...
    bLayer.setAttribute("width", diagramSettings.gridSize * zoomfact + "px");
    bLayer.setAttribute("height", diagramSettings.gridSize * zoomfact + "px");

    bLayer.children[0].setAttribute('d', `M ${diagramSettings.gridSize * zoomfact} 0 L 0 0 0 ${diagramSettings.gridSize * zoomfact}`);

    // Set width of origo line on the x axis
    bLayer = document.getElementById("origoX");
    bLayer.style.strokeWidth = diagramSettings.origoWidth * zoomfact;
...
function setTimerToMessage(element, time = 5000)
{
    ...
    var timer = setInterval( function(){
        var element = document.getElementById(diagramSettings.errorMsgMap[timer].id); // TODO : SAME VARIABLE NAME AS OUTER SCOPE?????
        diagramSettings.errorMsgMap[timer].percent -= 1;
        element.lastElementChild.style.width = `calc(${diagramSettings.errorMsgMap[timer].percent - 1}% - 10px)`;

        // If the time is out, remove the message
        if(diagramSettings.errorMsgMap[timer].percent === 0) removeMessage(element, timer);

    }, time / 100);

    // Adds to map: TimerID: ElementID, Percent
    diagramSettings.errorMsgMap[timer] = {
        id: element.id,
        percent: 100
    };
}
...
function removeMessage(element, timer)
{
    ...
    if (!timer) {
        timer = Object.keys(diagramSettings.errorMsgMap).find(key => {
            return diagramSettings.errorMsgMap[key].id === element.id
        });
    }

    if (timer) {
        clearInterval(timer); // Remove the timer
        delete diagramSettings.errorMsgMap[timer]; // Remove timer from the map
    }

    element.remove(); // Remove the element from DOM
    // Remove ID from randomidArray
    diagramSettings.randomidArray = diagramSettings.randomidArray.filter(id => {
...
var rulerSettings = {
    ...
    for (i = 100 + rulerSettings.zoomY; i <= pannedY -(pannedY *2) + cheight ; i += (rulerSettings.lineRatio*zoomfact)) {
        ...
        if (lineNumber === rulerSettings.fullLineRatio) {
            lineNumber = 0;
            rulerSettings.barY += "<line x1='0px' y1='"+(pannedY+i)+"' x2='40px' y2='"+(pannedY+i)+"' stroke='"+rulerSettings.color+"' />";
            rulerSettings.barY += "<text x='2' y='"+(pannedY+i+10)+"'style='font-size: 10px'>"+rulerSettings.cordY+"</text>";
            rulerSettings.cordY = rulerSettings.cordY +100;
        }else if (zoomfact > 0.5){
            rulerSettings.barY += "<line x1='25px' y1='"+(pannedY+i)+"' x2='40px' y2='"+(pannedY+i)+"' stroke='"+rulerSettings.color+"' />";
        } 
    }

    //Draw the Y-axis ruler negative side.
    lineNumber = (rulerSettings.fullLineRatio - 11);
    rulerSettings.cordY = -100;
    ...
    svgY.innerHTML = rulerSettings.barY; //Print the generated ruler, for Y-axis
    ...
    for (i = 51 + rulerSettings.zoomX; i <= pannedX - (pannedX *2) + cwidth; i += (rulerSettings.lineRatio*zoomfact)) {
        ...
        if (lineNumber === rulerSettings.fullLineRatio) {
            lineNumber = 0;
            rulerSettings.barX += "<line x1='" +(i+pannedX)+"' y1='0' x2='" + (i+pannedX) + "' y2='40px' stroke='" + rulerSettings.color + "' />";
            rulerSettings.barX += "<text x='"+(i+5+pannedX)+"'"+verticalText+"' y='15' style='font-size: 10px'>"+rulerSettings.cordX+"</text>";
            rulerSettings.cordX = rulerSettings.cordX +100;
        }else if (zoomfact > 0.5){
            rulerSettings.barX += "<line x1='" +(i+pannedX)+"' y1='25' x2='" +(i+pannedX)+"' y2='40px' stroke='" + rulerSettings.color + "' />";
        }
    }

    //Draw the X-axis ruler negative side.
    lineNumber = (rulerSettings.fullLineRatio - 11);
    rulerSettings.cordX = -100;
    for (i = -51 - rulerSettings.zoomX; i <= pannedX; i += (rulerSettings.lineRatio*zoomfact)) {
        ...
        if (lineNumber === rulerSettings.fullLineRatio) {
            lineNumber = 0;
            rulerSettings.barX += "<line x1='" +(pannedX-i)+"' y1='0' x2='" + (pannedX-i) + "' y2='40px' stroke='" + rulerSettings.color + "' />";
            rulerSettings.barX += "<text x='"+(pannedX-i+5)+"'"+verticalText+"' y='15'style='font-size: 10px'>"+rulerSettings.cordX+"</text>";
            rulerSettings.cordX = rulerSettings.cordX -100;
        }else if (zoomfact > 0.5){
            rulerSettings.barX += "<line x1='" +(pannedX-i)+"' y1='25' x2='" +(pannedX-i)+"' y2='40px' stroke='" + rulerSettings.color + "' />";
        }
    }
    svgX.style.boxShadow ="3px 3px 6px #5c5a5a";
    svgX.style.backgroundColor = "#e6e6e6";
    svgX.innerHTML = rulerSettings.barX;//Print the generated ruler, for X-axis
}
/**
...
function updateCSSForAllElements()
{
    function updateElementDivCSS(elementData, divObject, useDelta = false)
    {
        ...
        if (diagramSettings.snapToGrid && useDelta) {
            if (elementData.id === targetElement.id) {
                // The element coordinates with snap point
                var objX = Math.round((elementData.x - (deltaX * (1.0 / zoomfact))) / diagramSettings.gridSize) * diagramSettings.gridSize;
                var objY = Math.round((elementData.y - (deltaY * (1.0 / zoomfact))) / diagramSettings.gridSize) * diagramSettings.gridSize;
