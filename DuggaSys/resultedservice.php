...
if($requestType == "mail"){
	...
	// {
			$studentID = "Conny";
			$mailQuery = $pdo->prepare("SELECT user.email FROM user INNER JOIN user_course ON user.uid = user_course.uid WHERE user_course.cid=:cid AND user_course.vers=:cvers AND user.firstname =:studentID");
