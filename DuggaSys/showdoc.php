			...
			if(is_numeric($fid)){
				...
				if($row = $query->fetch(PDO::FETCH_ASSOC)){
					if(file_exists ( $row['filename'])){
						...
						$bummer = "<div class='err'><span style='font-weight:bold;'>Basdlkasndummer!</span> The link you asked for does not currently exist!</div>";
					}
				}else{
					...
					if($filekind==1){
						...
						echo "<script>window.location.replace('".$filename."');</script>";
							...
							if(file_exists ( $file)){
									...
									$bummer = "<div class='err'><span style='font-weight:bold;'>222222Bummer!</span> The link you asked for does not currently exist!".$file."</div>";
								}
					}
				}else{
					$bummer = "<div class='err'><span style='font-weight:bold;'>Bummer!</span> You have reached a non-navigable link!</div>";
