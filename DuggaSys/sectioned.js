		...
		if (data['entries'].length > 0) {
			...
			for(i=0;i<data['entries'].length;i++){
				...
				if(parseInt(item['kind']) === 0 ){									// Header
					...
					str += "<td ";
          if(kk%2==0){
						str+=" style='border:2px solid red;' class='example item"+blorf+"' placeholder='"+momentexists+"' id='I"+item['lid']+"' ";
					}else{
						str+=" style='border:2px solid red;' class='example item"+blorf+"' placeholder='"+momentexists+"' id='I"+item['lid']+"' ";
					}
          /*
