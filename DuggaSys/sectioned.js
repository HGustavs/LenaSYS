...
// Retrieve start and end dates for a version, if there are such, else set to null
      ...
      for ( i = 0; i < retdata['versions'].length; i++) {
        ...
        if(retdata["writeaccess"]){
            str += "<td><input type='button' value='+' class='submit-button-newitem' onclick='selectItem(\""+item['lid']+"\",\"New Item\",\""+item['kind']+"\",\""+item['visible']+"\",\""+item['link']+"\",\""+momentexists+"\",\""+item['gradesys']+"\",\""+item['highscoremode']+"\");showSubmitButton();'/>";
            str+="</tr></table>";
		...
		str+="<div class='course'>";
        	str+="<td style='display: inline-block; margin-right:2px; width:112px;'><input type='button' value='Show/Hide all' class='hide-moments-button'</td>";
