...
function selectItem(lid, entryname, kind, evisible, elink, moment, gradesys) {
	...
	// Set GradeSys
	str = "";
	if (gradesys == null || gradesys == 0)
		str += "<option selected='selected' value='0'>-</option>";
	else
		str += "<option value='0'>-</option>";
	if (gradesys == 1)
		str += "<option selected='selected' value='1'>U-G-VG</option>";
	else
		str += "<option value='1'>U-G-VG</option>";
	if (gradesys == 2)
		str += "<option selected='selected' value='2'>U-G</option>";
	else
		str += "<option value='2'>U-G</option>";
	if (gradesys == 3)
		str += "<option selected='selected' value='3'>U-3-4-5</option>";
	else
		str += "<option value='3'>U-3-4-5</option>";
	...
	if (retdata['entries'].length > 0) {
		...
		// Account for null
		if (moment == "")
			str += "<option selected='selected' value='null'>&lt;None&gt;</option>";
		else
			str += "<option value='null'>&lt;None&gt;</option>";

		// Account for rest of moments!
		for (var i = 0; i < retdata['entries'].length; i++) {
			var item = retdata['entries'][i];
			if (item['kind'] == 4) {
				if (parseInt(moment) == parseInt(item['lid']))
					str += "<option selected='selected' value='" + item['lid'] + "'>" + item['entryname'] + "</option>";
				else
					str += "<option value='" + item['lid'] + "'>" + item['entryname'] + "</option>";
			}
		}
	}
	$("#moment").html(str);
	...
	str = "";
	if (kind == 0)
		str += "<option selected='selected' value='0'>Header</option>";
	else
		str += "<option value='0'>Header</option>";
	if (kind == 1)
		str += "<option selected='selected' value='1'>Section</option>";
	else
		str += "<option value='1'>Section</option>";
	if (kind == 2)
		str += "<option selected='selected' value='2'>Code</option>";
	else
		str += "<option value='2'>Code</option>";
	if (kind == 3)
		str += "<option selected='selected' value='3'>Test</option>";
	else
		str += "<option value='3'>Test</option>";
	if (kind == 4)
		str += "<option selected='selected' value='4'>Moment</option>";
	else
		str += "<option value='4'>Moment</option>";
	if (kind == 5)
		str += "<option selected='selected' value='5'>Link</option>";
	else
		str += "<option value='5'>Link</option>";
	$("#type").html(str);
				...
				if (parseInt(item['visible']) === 1 || data['writeaccess']) {
					...
					if (kk == 1) {
						if (parseInt(item['visible']) === 0)
							str += " style='opacity: 0.5; box-shadow: 0px 3px 2px #aaa inset; border-radius:8px; margin-left:4px;' ";
						else
							str += "style='box-shadow: 0px 3px 2px #aaa inset;' ";
					} else {
						if (parseInt(item['visible']) === 0)
							str += " style='opacity: 0.5; border-radius:8px; margin-left:4px;' ";
					}
					...
					if (data['writeaccess'])
						str += "<img id='dorf' style='float:right;margin-right:8px' src='css/svg/Cogwheel.svg' onclick='selectItem(\"" + item['lid'] + "\",\"" + item['entryname'] + "\",\"" + item['kind'] + "\",\"" + item['visible'] + "\",\"" + item['link'] + "\",\"" + item['moment'] + "\",\"" + item['gradesys'] + "\");' />";
					...
					if (parseInt(item['kind']) === 3 || parseInt(item['kind']) === 4) {
						...
						if (grady == -1) {
							...
							str += "<img id='korf' style='float:left;margin-left:8px' title='Status: Not Handed In' src='css/svg/StopN.svg' />";
						} else if (grady == null) {
