    ...
    if (retdata['versions'].length > 0) {
      for ( j = 0; j < retdata['versions'].length; j++) {
		...
		if(data['writeaccess']) {
			...
			str+="<td style='width:112px;'><input type='button' value='Groups' class='submit-button' title='Assign students to groups page' onclick='changeURL(\"grouped.php?cid="+querystring['courseid']+"&coursevers="+querystring['coursevers']+"\")'/></td>";
