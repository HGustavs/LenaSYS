...
function returnedCourse(data)
{
	...
	if (data['writeaccess']){
		str+="<img alt='image of cogwheel' style='margin-left:17px;cursor:pointer;' src='../Shared/icons/Cogwheel.svg' onclick='editSettings(); 'title='Edit Server Settings'>"
	}
	str+="</div>";
		...
		for ( i = 0; i < data['entries'].length; i++) {
			...
			if (data['writeaccess']) {
        		...
        		str += "<span style='margin-bottom: 0px'>";
				    ...
				    str += "<span><img alt='image of cogwheel' id='dorf' style='position: relative; top: 2px;' src='../Shared/icons/Cogwheel.svg' onclick='selectCourse(\"" + item['cid'] + "\",\"" + htmlFix(item['coursename']) + "\",\"" + item['coursecode'] + "\",\"" + item['visibility'] + "\",\"" + item['activeversion'] + "\",\"" + item['activeedversion'] + "\");' title='Edit \"" + item['coursename'] + "\" '></span>";
